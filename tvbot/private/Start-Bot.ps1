function Start-Bot {
    ...
    $menurestart.add_Click( {
            Stop-Process $script:newprocess.Id
            $script:newprocess = Start-Process -FilePath powershell -ArgumentList "-NoLogo -NoProfile -Command Start-TvBot -NoTrayIcon -PrimaryPid $PID $script:flatparams" -PassThru
